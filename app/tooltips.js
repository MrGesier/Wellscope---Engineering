...
const HELP={
  ...
  bitforce:'Axial force boundary condition applied at the bit, in kN, for the bottom-up screening calculation. This input is not automatically a physically correct WOB, motor thrust or weight-on-bit estimate for every operating mode.',
  hookref:'Optional entered observed/reference surface hookload (kN) shown alongside the preliminary model output. This alpha does not fit friction to measured pickup/slackoff or evaluate model residuals.',
  hookcap:'Optional nominal rig/equipment hookload rating (kN), shown for information only. It is NOT the allowable string load or a safety limit; actual limits depend on rig, hoisting system, connections, design factors and approved procedures.',
  ...
  tdplot:'Local axial load (kN) against reference MD for pickup, slackoff and static modes. Curve differences are generated by a simplified friction sign; this is not a validated hookload envelope.',
  ...
  k_pu:'Surface pickup hookload estimate (kN) for pulling the string in the simplified segmented soft-string model. This is not a measured or allowable rig load.',
  k_so:'Surface slackoff hookload estimate (kN) for lowering the string in the simplified axial model. The model is not calibrated to field friction.',
  k_st:'Static surface axial-load estimate (kN) without a movement-direction friction term. It is not an actual measured hookload and excludes numerous dynamic/pressure effects.',
