...
function typeOf(name){let n=String(name).toLowerCase();if(n.includes('tricone')||n.includes('roller'))return 'tricone';if(n.includes('stabilizer'))return 'stab';if(n.includes('reamer'))return 'reamer';if(n.includes('pdc')||n.includes('tricone bit')||n.includes('trépan'))return 'pdc';if(n.includes('motor')||n.includes('pdm'))return 'motor';if(n.includes('rss'))return 'rss';if(n.includes('stab'))return 'stab';if(n.includes('mwd')||n.includes('lwd'))return 'mwd';if(n.includes('jar'))return 'jar';if(n.includes('ream'))return 'reamer';if(n.includes('hwdp'))return 'hwdp';if(n.includes('nmdc')||n.includes('collar'))return 'nmdc';return 'dp';}
function svg(type){if(root.EquipmentDrawing)return root.EquipmentDrawing.svg(({pdc:'pdc-bit',motor:'bent-pdm',rss:'rss-push',stab:'string-stabilizer',dp:'drillpipe'})[type]||type); const steel='#779cb5',line='#b5d3df',a='#52dbc8',gold='#ffc078';let shape=`<rect x="10" y="23" width="78" height="18" rx="3" fill="${steel}" stroke="${line}"/><path d="M24 24v16 M74 24v16" stroke="#33506a"/>`;
switch(type){
